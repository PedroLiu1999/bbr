...
function disable_bbr() {
    # 1. Check if BBR is actually enabled
    current_cc=$(sysctl net.ipv4.tcp_congestion_control | awk '{print $3}')
    if [[ "$current_cc" != "bbr" ]]; then
        echo -e "${RED}BBR is not currently enabled.${NC}"
        return
    fi

    echo -e "Disabling BBR and restoring settings..."
    
    # 2. Try to restore from backup first (KEEPING the backup file)
    if [[ -f "$BACKUP_FILE" ]]; then
        source "$BACKUP_FILE"
        ...
        echo -e "${RED}Warning: No backup file found. Using standard defaults.${NC}"
        sysctl -w net.core.default_qdisc=fq_codel > /dev/null 2>&1
        sysctl -w net.ipv4.tcp_congestion_control=cubic > /dev/null 2>&1
    fi
    
    # 3. Reload settings from file (Reloading BEFORE removal)
    echo -e "Reloading system settings..."
    sysctl -p > /dev/null
    
    # 4. Remove BBR lines from sysctl.conf ONLY AFTER reload is completed
    sed -i '/net.core.default_qdisc/d' /etc/sysctl.conf
    sed -i '/net.ipv4.tcp_congestion_control/d' /etc/sysctl.conf
